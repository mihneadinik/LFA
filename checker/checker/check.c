...
int **read_dfa(FILE *dfa, int *n, int *m, int *s) {
    fscanf(dfa, "%d%d%d", n, m, s);
...
int check_sync(FILE *seq, int **conn, int *starts, int s, int m) {
    int symbol;
    while(1) {
        fscanf(seq, "%i", &symbol);
        if(feof(seq)) {
...
int main (int argc, char **argv) {
    ...
    int n, m, s;
    int **conn = read_dfa(fd, &n, &m, &s);

    int *starts = read_states(fd, &s, n);

    return check_sync(fs, conn, starts, s, m);
}
